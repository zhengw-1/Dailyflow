...
function renderTask(task){
  ...
  const datePill=overdue
    ? `<span class="pill overdue">Overdue · Due ${task.due}</span>`
    : `<span class="pill blue">${range}</span>`;
  return `
  <div class="task ${isTaskDone(task)?"completed":""}" data-id="${task.id}" ondragover="allowTaskDrop(event,'${task.id}')" ondragleave="clearTaskDrop(event)" ondrop="dropTask(event,'${task.id}')">
    <div class="task-row">
