...
const defaults = [
  ...
  {id:crypto.randomUUID(),title:"Organize portfolio images",start:getToday(),due:plusDays(3),category:"Design",priority:"Medium",reminder:"Off",focus:true,completed:false,notes:"",subtasks:[
...
function renderTask(task){
  ...
  const range=task.start===task.due ? task.due : `${task.start} → ${task.due}`;
  const overdue=DailyFlowOrder.isOverdue(task,selectedDate);
  const datePill=overdue
    ? `<span class="pill blue due-emphasis">Due ${task.due}</span>`
    : `<span class="pill blue">${range}</span>`;
  return `
  <div class="task ${isTaskDone(task)?"completed":""} ${overdue?"overdue-card":""}" data-id="${task.id}" ondragover="allowTaskDrop(event,'${task.id}')" ondragleave="clearTaskDrop(event)" ondrop="dropTask(event,'${task.id}')">
    <div class="task-row">
      ...
      <div>
        ...
        <div class="meta">
          <span class="pill purple">${escapeHtml(task.category)}</span>
          ...
          <span class="pill ${task.priority==="High"?"high":""} inline-control-pill">
          ...
          <span class="pill inline-control-pill">
            <select class="inline-select" title="Change status" onchange="inlineStatus('${task.id}',this.value)">
              <option value="active" ${!isTaskDone(task)?"selected":""}>Active</option>
              <option value="completed" ${isTaskDone(task)?"selected":""}>Checked</option>
            </select>
          </span>
...
window.inlineStatus=(id,value)=>{
  if(DailyFlowInlineEdit.updateStatus(tasks,id,value)){save();render();showToast(value==="completed"?"Task checked":"Task moved back to active");}
};
...
function openModal(task=null){
  ...
  if(task){
    ...
    document.getElementById("category").value=[...document.getElementById("category").options].some(o=>o.value===task.category)?task.category:"Other";
...
document.getElementById("saveTask").onclick=()=>{
  ...
  const data={
    title,start,due,category:document.getElementById("category").value,
